...
mas_x=[1.340,1.345,1.350,1.255,1.160,1.365,1.370,1.375,1.380,1.385,1.390]
mas_y=[4.00556 ,4.6532, 4.4552, 4.5618, 4.6734, 4.4903, 4.9130, 5.0419, 5.1774, 5.3201, 5.4706]
h = mas_x[1] - mas_x[0]
x=0.1
xo=0
xn=1.390
...
print ("1 інтерполяційна  формула Ньютона: //", Nx1, " // (через цикл до)", jkl)
print ("1 інтерполяційна  формула Ньютона: //", N1," // (через Формулу до 3 порядка)")
print ("2 інтерполяційна  формула Ньютона: //", Nx2, " // (через цикл до)", jkl)
plt.plot(mas_x, mas_y, label='Nx1 , Nx2')
plt.title('9_lb Anton Shyshka')
